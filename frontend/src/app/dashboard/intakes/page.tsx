...
import { 
  ...
  Microscope, TestTubes, DollarSign, AlertCircle
} from "lucide-react";
...
// ==========================================
// ERROR FORMATTER (Prevents App Crashes)
// ==========================================
...
// ==========================================
// TYPES & ENUMS (Mapped to FastAPI Pydantic)
// ==========================================
...
export default function IntakesPage() {
  ...
  // ==========================================
  // BILLING CALCULATOR (Frontend Preview)
  // ==========================================
  ...
  const getStatusColor = (status: IntakeStatusEnum) => {
    ...
    return 'bg-amber-50 text-amber-800 border-amber-200/80 ring-amber-500';
  };
    ...
    <div className="flex-1 flex flex-col min-h-screen animate-in fade-in duration-500 font-sans pb-24 relative">
      ...
      <header className="mb-6 lg:mb-8">
        <div className="flex items-center gap-2.5 text-teal-600 mb-2">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest">Active Operations • {activeLab || "Workspace"}</span>
        </div>
      ...
      {/* 2. UNIFIED COMMAND BAR (Test Directory Style) */}
      <div className="bg-white p-2.5 rounded-[1.25rem] border border-slate-200/80 shadow-sm flex flex-col lg:flex-row items-center justify-between gap-3 mb-8 relative z-20">
        ...
        <div className="relative w-full lg:flex-1 flex items-center bg-slate-50 rounded-xl border border-slate-200/80 focus-within:border-teal-500 focus-within:ring-4 focus-within:ring-teal-500/10 transition-all">
          ...
          <input
            ...
            placeholder="Search Accession, Name, Phone..."
            value={searchQuery}
        ...
        <div className="flex items-center gap-2 w-full lg:w-auto">
          <div className="flex bg-slate-50 p-1 rounded-xl border border-slate-200/80 flex-1 lg:flex-none overflow-x-auto no-scrollbar">
            {(["All", "registered", "processing", "completed"] as const).map((status) => (
              <button
                key={status}
              ...
              >
                {status === "All" ? "All Queue" : formatEnum(status)}
              </button>
            ))}
          </div>
          ...
          <button
            ...
            className="bg-teal-600 hover:bg-teal-700 text-white px-5 py-3 rounded-xl text-sm font-black shadow-lg shadow-teal-500/20 transition-all active:scale-95 shrink-0 outline-none flex items-center gap-2"
          >
            <Plus size={16} strokeWidth={3} /> <span className="hidden sm:inline">New Intake</span>
          </button>
        </div>
      </div>
      ...
      {isLoadingQueue ? (
        ...
        <>
            ...
            {filteredIntakes.map((patient) => (
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button 
                    onClick={(e) => handleToggleStatus(e, patient.id, patient.status)}
                    className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-black uppercase tracking-widest rounded-lg border shadow-sm transition-all hover:scale-105 active:scale-95 ${getStatusColor(patient.status)}`}
                  >
                    <span className={`relative flex h-1.5 w-1.5 shrink-0 ${patient.status !== 'completed' && patient.status !== 'cancelled' ? 'animate-pulse' : ''}`}>
                      <span className={`relative inline-flex rounded-full h-1.5 w-1.5 ring-2 ring-current ring-inset bg-current`} />
                    </span>
                    <span>{formatEnum(patient.status)}</span>
                  </button>
                </div>
          ...
          {/* B. DESKTOP TABLE (>= lg) */}
          <div className="hidden lg:block bg-white rounded-[2rem] border border-slate-200/80 shadow-sm overflow-visible mb-12 relative z-10">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/80 border-b border-slate-200/80 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">
                  ...
                  <th className="py-5 px-6">Billing & Ledger</th>
        ...
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full md:w-[550px] h-full bg-slate-50 shadow-2xl border-l border-slate-200 flex flex-col animate-in slide-in-from-right duration-500 ease-[cubic-bezier(0.2,0.8,0.2,1)] z-10">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input 
                        ...
                        className="w-full py-3 px-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 placeholder:text-slate-400 focus:bg-white focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 outline-none transition-all shadow-sm" 
                    ...
                    <div>
                      ...
                      <div className="relative flex items-center">
                        ...
                        <input 
                          ...
                          className="w-full py-3 pl-11 pr-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 placeholder:text-slate-400 focus:bg-white focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 outline-none transition-all shadow-sm" 
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input 
                          type="number" min="0" max="120" value={age} onChange={(e) => setAge(e.target.value ? Number(e.target.value) : "")}
                          className="w-full py-3 px-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 placeholder:text-slate-400 focus:bg-white focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 outline-none transition-all shadow-sm" 
                          placeholder="Years" 
                        />
                      </div>
                      <div>
                        ...
                        <div className="relative">
                          <select 
                            ...
                            className="w-full py-3 pl-4 pr-8 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 focus:bg-white focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 outline-none appearance-none cursor-pointer shadow-sm"
                          >
                ...
                <div className="bg-white p-5 rounded-[2rem] border border-slate-200/80 shadow-sm space-y-5">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="relative" ref={dropdownRef}>
                      ...
                      <div className="relative flex items-center">
                        {isFetchingTests ? <Loader2 size={16} className="absolute left-4 text-teal-600 animate-spin" /> : <Search size={16} className="absolute left-4 text-slate-400" />}
                        <input 
                          type="text" 
                          className="w-full py-3 pl-11 pr-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 placeholder:text-slate-400 focus:bg-white focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 outline-none transition-all shadow-sm" 
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <div className="relative flex-1">
                          ...
                          <input 
                            type="number" min="0" value={discountValue} onChange={(e) => setDiscountValue(e.target.value ? Number(e.target.value) : "")}
                            placeholder={discountType === "percentage" ? "Enter % (e.g. 10)" : "Enter Flat ₹"}
                            className="w-full pl-9 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:bg-white focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 outline-none transition-all shadow-sm"
                          />
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <select
                          ...
                          className="w-full py-2.5 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 outline-none focus:bg-white focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 cursor-pointer shadow-sm transition-all"
                        >
                      ...
                      <div>
                        ...
                        <select
                          ...
                          className="w-full py-2.5 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 outline-none focus:bg-white focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 cursor-pointer disabled:opacity-50 shadow-sm transition-all"
                        >
                    ...
                    {paymentStatus === "partial" && (
                      <div className="animate-in fade-in slide-in-from-top-2">
                        <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Deposit Amount Received (₹)</label>
                        <input 
                          type="number" min="1" max={finalTotal - 1} value={paidAmountInput} onChange={(e) => setPaidAmountInput(e.target.value ? Number(e.target.value) : "")}
                          className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:outline-none focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 focus:bg-white transition-all shadow-sm"
            ...
            <div className="p-5 border-t border-slate-200/80 bg-white/95 backdrop-blur-md flex justify-end gap-3 shrink-0 sticky bottom-0 z-20">
              <button type="button" onClick={resetIntakeForm} className="px-6 py-3 text-sm font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors outline-none shadow-sm">
                Cancel
              </button>
              <button 
                type="button" onClick={handleSubmitIntake} disabled={selectedTests.length === 0 || !fullName || !phone || isSubmitting || (paymentStatus === "partial" && !paidAmountInput)}
              ...
              >
                {isSubmitting ? <Loader2 size={18} className="animate-spin" /> : <CreditCard size={18} strokeWidth={2.5} />}
      ...
      {/* ========================================================= */}
      {/* 5. COLLECT BALANCE POPUP MODAL (PUT /intakes/{id})        */}
      {/* ========================================================= */}
          ...
          <div className="relative w-full max-w-md bg-white rounded-[2rem] p-6 shadow-2xl space-y-5 animate-in zoom-in-95 border border-slate-200/80">
            ...
            <form onSubmit={handleCollectBalanceSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Payment Method</label>
                <select 
                  value={additionalPayMethod} onChange={(e) => setAdditionalPayMethod(e.target.value as PaymentMethodEnum)}
                  className="w-full py-3 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 outline-none focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 focus:bg-white transition-all cursor-pointer shadow-sm"
                >
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5">Collection Amount (₹)</label>
                <input 
                  ...
                  className="w-full py-3 px-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 outline-none focus:bg-white focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 transition-all shadow-sm"
                />
              </div>

              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={() => setCollectBalanceRecord(null)} className="px-5 py-2.5 text-xs font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors rounded-xl outline-none shadow-sm">
                  Cancel
                </button>
                <button 
                  type="submit" disabled={!additionalPayment || isCollectingBalance}
                  className="px-6 py-2.5 text-xs font-black text-white bg-teal-600 hover:bg-teal-700 transition-all active:scale-95 rounded-xl shadow-lg shadow-teal-500/20 flex items-center gap-1.5 disabled:opacity-50 outline-none"
                >
                  {isCollectingBalance ? <Loader2 size={14} className="animate-spin" /> : <DollarSign size={14} strokeWidth={3} />}
                  Record Receipt
                </button>
              </div>
            </form>
...
// // Exact match to Backend `IntakeResponse`
// interface IntakeRecord {
//   id: string;
//   lab_id: string;
//   accession_number: string;
...
// // UI Formatter for Enums (e.g., "sample_collected" -> "Sample Collected")
// const formatEnum = (val?: string) => {
//   if (!val) return "";
//   return val.replace(/_/g, " ").replace(/\b\w/g, l => l.toUpperCase());
// };
...
//   // Balance Payment Collection Modal State
//   const [collectBalanceRecord, setCollectBalanceRecord] = useState<IntakeRecord | null>(null);
//   const [additionalPayment, setAdditionalPayment] = useState<number | "">("");
//   const [additionalPayMethod, setAdditionalPayMethod] = useState<PaymentMethodEnum>("upi");
//   const [isCollectingBalance, setIsCollectingBalance] = useState(false);
...
//       return Math.min(subtotal, Math.round((subtotal * num) / 100));
//     }
//     return Math.min(subtotal, num);
//   }, [subtotal, discountType, discountValue]);
...
//     if (status === 'sample_collected') return 'bg-indigo-50 text-indigo-800 border-indigo-200/80 ring-indigo-500';
//     if (status === 'cancelled') return 'bg-rose-50 text-rose-800 border-rose-200/80 ring-rose-500';
//     return 'bg-amber-50 text-amber-800 border-amber-200/80 ring-amber-500'; // Registered/Pending
//   };
...
//           <div className="flex bg-slate-100/80 p-1.5 rounded-2xl w-full sm:w-auto border border-slate-200/50 shadow-inner overflow-x-auto no-scrollbar">
//             {(["All", "registered", "processing", "completed"] as const).map((status) => (
//               <button
//                 key={status}
//                 onClick={() => setStatusFilter(status as any)}
//                 className={`flex-1 sm:flex-initial px-6 py-2 text-xs font-bold rounded-xl transition-all whitespace-nowrap outline-none ${
//                   statusFilter === status 
//                     ? "bg-white text-slate-900 shadow-sm border border-slate-200/80" 
//                     : "text-slate-500 hover:text-slate-700 hover:bg-slate-200/50 border border-transparent"
//                 }`}
...
//       {/* 3. PATIENTS DATA GRID */}
//       {isLoadingQueue ? (
//         <div className="flex-1 flex flex-col items-center justify-center py-20 bg-white/50 rounded-3xl border border-slate-200/80 backdrop-blur-sm">
//            <Loader2 className="w-10 h-10 text-teal-600 animate-spin mb-4" />
//            <p className="text-slate-500 font-semibold text-sm">Loading operations queue...</p>
//         </div>
//       ) : (
...
//                       <span className="text-base font-black font-mono text-slate-900 block">₹{Number(patient.net_amount).toLocaleString('en-IN')}</span>
//                       {Number(patient.balance_due) > 0 && (
//                         <span className="text-[9px] font-mono font-bold text-rose-600 bg-rose-50 px-1.5 py-0.5 rounded border border-rose-200 shadow-sm">
//                           Due: ₹{Number(patient.balance_due).toLocaleString('en-IN')}
//                         </span>
//                       )}
//                     </div>
...
//                         <button onClick={(e) => { e.stopPropagation(); router.push(`/dashboard/reports/${patient.id}`); }} className="w-full text-left px-4 py-2.5 text-xs font-bold text-slate-700 hover:bg-teal-50 hover:text-teal-700 flex items-center gap-2.5 transition-colors">
//                           <FileText size={14} className="text-slate-400" /> Enter Metrics
//                         </button>
...
//                           <button 
//                             onClick={(e) => { e.stopPropagation(); setCollectBalanceRecord(patient); setOpenActionId(null); }} 
//                             className="w-full text-left px-4 py-2.5 text-xs font-bold text-teal-700 hover:bg-teal-50 flex items-center gap-2.5 transition-colors"
//                           >
//                             <DollarSign size={14} className="text-teal-600" /> Collect Balance
//                           </button>
//                         )}
...
//                         <button 
//                           onClick={(e) => handleDeleteIntake(e, patient.id, patient.patient_name)} 
//                           className="w-full text-left px-4 py-2.5 text-xs font-bold text-rose-600 hover:bg-rose-50 flex items-center gap-2.5 transition-colors rounded-b-2xl"
//                         >
//                           <Trash2 size={14} className="text-rose-400" /> Cancel Order
//                         </button>
//                       </div>
...
//                           <button type="button" onClick={() => setDiscountType("percentage")} className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${discountType === "percentage" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500"}`}>%</button>
//                           <button type="button" onClick={() => setDiscountType("flat")} className={`px-3 py-1.5 text-xs font-bold rounded-lg transition-all ${discountType === "flat" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500"}`}>₹</button>
//                         </div>
...
//                           value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value as any)}
//                           disabled={paymentStatus === "unpaid"}
//                           className="w-full py-2.5 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 outline-none cursor-pointer disabled:opacity-50"
//                         >
//                           <option value="upi">UPI / QR</option>
//                           <option value="cash">Cash</option>
//                           <option value="card">Credit/Debit Card</option>
...
//                         <input 
//                           type="number" min="1" max={finalTotal - 1} value={paidAmountInput} onChange={(e) => setPaidAmountInput(e.target.value ? Number(e.target.value) : "")}
//                           className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 focus:bg-white transition-all"
//                           placeholder={`Max: ₹${finalTotal}`}
//                         />
//                       </div>
//                     )}
...
//             {/* Drawer Footer Actions */}
//             <div className="p-5 border-t border-slate-200/80 bg-white/95 backdrop-blur-md flex justify-end gap-3 shrink-0 sticky bottom-0 z-20">
//               <button type="button" onClick={resetIntakeForm} className="px-6 py-3 text-sm font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors outline-none">
//                 Cancel
//               </button>
//               <button 
//                 type="button" onClick={handleSubmitIntake} disabled={selectedTests.length === 0 || !fullName || !phone || isSubmitting || (paymentStatus === "partial" && !paidAmountInput)}
...
//       {collectBalanceRecord && (
//         <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
//           <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm" onClick={() => setCollectBalanceRecord(null)} />
//           <div className="relative w-full max-w-md bg-white rounded-3xl p-6 shadow-2xl space-y-5 animate-in zoom-in-95">
            
//             <div className="flex justify-between items-center border-b border-slate-100 pb-3">
//               <div>
//                 <h3 className="text-base font-black text-slate-900">Collect Outstanding Balance</h3>
//                 <p className="text-xs text-slate-500 font-semibold">{collectBalanceRecord.accession_number} • {collectBalanceRecord.patient_name}</p>
//               </div>
//               <button onClick={() => setCollectBalanceRecord(null)} className="p-1.5 text-slate-400 hover:text-slate-600 rounded-xl transition-colors outline-none">
//                 <X size={18} strokeWidth={2.5} />
//               </button>
//             </div>

//             <form onSubmit={handleCollectBalanceSubmit} className="space-y-4">
//               <div className="bg-amber-50 border border-amber-200 rounded-xl p-3.5 flex justify-between items-center text-xs">
//                 <span className="font-bold text-amber-800">Remaining Balance:</span>
//                 <span className="font-mono font-black text-base text-amber-900">₹{Number(collectBalanceRecord.balance_due).toLocaleString("en-IN")}</span>
//               </div>
...
//                 <select 
//                   value={additionalPayMethod} onChange={(e) => setAdditionalPayMethod(e.target.value as PaymentMethodEnum)}
//                   className="w-full py-3 px-3 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 outline-none focus:ring-2 focus:ring-teal-500/20 focus:bg-white transition-all cursor-pointer"
//                 >
//                   <option value="upi">UPI / QR Code</option>
//                   <option value="cash">Cash</option>
//                   <option value="card">Credit/Debit Card</option>
...
//                   value={additionalPayment} onChange={(e) => setAdditionalPayment(e.target.value ? Number(e.target.value) : "")}
//                   placeholder={`Max: ₹${collectBalanceRecord.balance_due}`}
//                   className="w-full py-3 px-4 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-900 outline-none focus:bg-white focus:border-teal-500 focus:ring-4 focus:ring-teal-500/10 transition-all"
//                 />
//               </div>

//               <div className="flex justify-end gap-3 pt-2">
//                 <button type="button" onClick={() => setCollectBalanceRecord(null)} className="px-5 py-2.5 text-xs font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors rounded-xl outline-none">
//                   Cancel
//                 </button>
//                 <button 
//                   type="submit" disabled={!additionalPayment || isCollectingBalance}
//                   className="px-6 py-2.5 text-xs font-black text-white bg-teal-600 hover:bg-teal-700 transition-all rounded-xl shadow flex items-center gap-1.5 disabled:opacity-50 outline-none"
//                 >
//                   {isCollectingBalance ? <Loader2 size={14} className="animate-spin" /> : <DollarSign size={14} />}
//                   Record Receipt
//                 </button>
//               </div>
//             </form>
